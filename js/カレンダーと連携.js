...
function deleteChecked() {
	...
	const options = { method: "patch", headers: NOTION_REQUEST_HEADERS, payload: JSON.stringify({ archived: true }) };
...
const deleteCalendarEvent = (existEvents, tasks) => {
	const notionDataList = tasks.map((task) => createTaskData(task));
...
function syncNotion() {
	...
	deleteChecked();
	deleteCalendarEvent(existEvents, tasks);
